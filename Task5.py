...
import sys
sys.path.append('/root/thuei-1/sdk-python/')
import fo_black_line_grasp as fobl
fobl.black_line()
''',
                'red': '''
import sys
...
'''
    ...
    def _stop_conflicting_processes(self, starting_thread):
        ...
        # 定义冲突关系：哪些功能不能同时运行
        conflicts = {
            'line': ['line', 'color', 'qr', 'grasp','jinmen'],
            'color': ['line', 'color', 'qr', 'grasp','jinmen'], 
            'qr': ['line', 'color', 'qr', 'grasp','jinmen'],
            'grasp': ['line', 'color', 'qr', 'grasp','jinmen'],
            'jinmen': ['line', 'color', 'qr', 'grasp','jinmen'],
            'ultrasonic': ['ultrasonic','csfz'],
            'csfz': ['ultrasonic','csfz']  # 超声波可以和其他功能共存
        }
